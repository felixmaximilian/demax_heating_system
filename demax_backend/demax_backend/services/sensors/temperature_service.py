# -*- coding: utf-8 -*-
# from https://github.com/pingpan2013/sensor-box-project
__version__ = "0.01.03"
...
class DS18B20(object):
    ...
    SLAVE_PREFIX = "28-"
    SLAVE_FILE = "w1_slave"
    UNIT_FACTORS = {DEGREES_C: lambda x: x * 0.001, DEGREES_F: lambda x: x * 0.001 * 1.8 + 32.0,
    ...
    def get_sensor(cls, sensor_id):
        """Returns an instance DS18B20 for the sensor with id sensor_id"""
        sensor = DS18B20(sensor_id)
        if sensor.exists():
            return sensor
        else:
            raise ValueError("Sensor with id %s does not exist." % sensor_id)

    def __init__(self, sensor_id=None, load_kernel_modules=False):
...
class Hysterese:
    ...
    def is_in_upper_alarm(self, temperature):
        return temperature > self.upper

    def is_in_lower_alarm(self, temperature):
        return temperature < self.lower
